"""Built-in bank transaction template rules."""
...
def match_template(bank_name: str, sheet_name: str) -> BankTemplate | None:
    """Find best template by bank/sheet keywords."""
    bank_norm = _n(bank_name)
    sheet_norm = _n(sheet_name)
    # Priority 1: bank + sheet both hit.
    for item in BUILTIN_TEMPLATES:
        ...
        if hit_bank and hit_sheet:
            return item
    # Priority 2: when bank name is generic (e.g. "银行数据"), fall back to sheet-only matching.
    for item in BUILTIN_TEMPLATES:
        hit_sheet = any(_n(k) in sheet_norm for k in item.sheet_keywords if k)
        if hit_sheet:
            return item
...
def infer_bank_name(file_name: str, sheet_names: list[str], fallback: str = "银行数据") -> str:
    """Infer bank display name from filename/sheet names."""
    file_norm = _n(file_name)
    sheet_norms = [_n(x) for x in sheet_names if x]
    for bank_display, file_tokens, sheet_tokens in BANK_HINTS:
...
def match_template_by_columns(columns: list[str]) -> BankTemplate | None:
...
def infer_bank_name_by_columns(columns: list[str], fallback: str = "银行数据") -> str:
