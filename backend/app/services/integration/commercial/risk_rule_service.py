...
from app.services.integration.commercial.export_service import CommercialExportService, _strip_company_q_suffix
...
def _split_company_names(text: str) -> list[str]:
    """Split cells that contain multiple company names into independent companies."""
    raw = (text or "").strip()
    if not raw:
        return []
...
class CommercialRiskAnalysisService:
    ...
    def _build_context(self, rows: list[dict[str, str]]) -> dict[str, Any]:
        ...
        for row in rows:
            ...
            if inq and win_raw:
                for winner_name in _split_company_names(win_raw):
                    wn = _norm_company(winner_name)
                    if not wn:
                        continue
                    inquiry_winners[inq].add(wn)
                    # 单项目多家公司中标时不作为“唯一中标方”参与轮换序列。
                    if len(inquiry_winners[inq]) == 1:
                        inquiry_winner[inq] = wn
                    else:
                        inquiry_winner.pop(inq, None)
                    win_amt = _safe_float(row.get("中标金额(元)", ""))
                    if win_amt > 0 and inq not in inquiry_winner_amount:
                        inquiry_winner_amount[inq] = win_amt
            if inq and co:
                for company_name in _split_company_names(co):
                    ...
                    if cn not in company_display_name:
                        company_display_name[cn] = company_name
    ...
    def _rule_r006(
        ...
        ordered_all: list[str] = ctx["ordered_inquiries"]
        inquiry_winners: dict[str, set[str]] = ctx.get("inquiry_winners", {})
        unique_pairs: list[tuple[str, str]] = []
        skipped_multi: list[str] = []
        for inq in ordered_all:
            winner_set = inquiry_winners.get(inq, set())
            if len(winner_set) > 1:
                skipped_multi.append(inq)
                continue
            winner = ctx["inquiry_winner"].get(inq, "")
            if winner:
                unique_pairs.append((inq, winner))
        ordered = [inq for inq, _winner in unique_pairs]
        winners = [winner for _inq, winner in unique_pairs]
        for i in range(len(winners)):
            ...
            if len(distinct) >= win_need:
                inq_chunk = ordered[i : i + window]
                ev = {
                    "口径": "连续多单唯一中标方在固定集合内轮换（已排除单项目多中标方）",
                    "窗口": window,
                    "窗口内不同中标方数": len(distinct),
                    "中标方序列片段": chunk,
                    "对应询价单号": inq_chunk,
                    "已排除多中标方询价单": skipped_multi[:20],
                }
                for winner in sorted(distinct):
                    self._insert_event(batch_id, "R006", title, weight, winner, inq_chunk[0], ev)
