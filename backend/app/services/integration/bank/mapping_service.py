...
class BankMappingService:
    ...
    def _standardize_table(
        ...
        for row in raw_rows:
            ...
            direction = self._normalize_direction_with_rules(
                std_fields.get("txn_direction"), txn_amount, direction_rules
            )
            if self._looks_like_non_transaction_row(std_fields, payload_dict, txn_time, txn_amount):
                continue
            ...
            raw_remark = (std_fields.get("remark") or "").strip()
            source_name = self._build_source_name(row[2], row[3], raw_table_name)
            risk_remark = self._build_risk_remark(txn_time, txn_amount, summary)
            remark = self._join_remark_parts(raw_remark, risk_remark)
    ...
    def _looks_like_non_transaction_row(
        self,
        std_fields: dict[str, str | None],
    ...
    def clear_field_mappings(self, template_fingerprint: str) -> None:
        """Remove all field mappings for a fingerprint (next standardize re-seeds)."""
        self._client.execute(
    ...
    @staticmethod
    def _direction_rule_key(text: str) -> str:
    ...
    def _normalize_direction_with_rules(
        self, direction: str | None, amount: str, rules: dict[str, str]
    ) -> str:
        text = (direction or "").strip()
        if text and rules:
            for k in (text, self._direction_rule_key(text)):
                if k and k in rules:
                    mapped = rules[k]
                    if mapped in ("收入", "支出", "未知"):
                        return mapped
        return self._normalize_direction(direction, amount)

    def _normalize_txn_time_extended(
        self, txn_date: str | None, txn_time_raw: str | None, rules: dict[str, Any] | None
    ) -> str:
        if not rules or not isinstance(rules.get("formats"), list) or not rules["formats"]:
            return self._normalize_txn_time(txn_date, txn_time_raw)
        date_text = (txn_date or "").strip()
        time_text = (txn_time_raw or "").strip()
        raw = time_text if time_text else date_text
        if not raw:
    ...
    def _join_remark_parts(self, *parts: str) -> str:
        """Join user remark and generated tags without duplicates."""
        out: list[str] = []
        seen: set[str] = set()
        for part in parts:
    ...
    def _resolve_row_bank_name(
        ...
        return infer_bank_name(file_name, [sheet], fallback=fallback_bank, client=self._client)
