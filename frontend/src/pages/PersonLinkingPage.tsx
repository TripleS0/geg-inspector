...
function PersonLinkingPage() {
  ...
  const refreshPersons = useCallback(async (id: number) => {
    try {
      ...
      if (msg.startsWith("404")) {
        ...
        setCases(caseList.items.map((c) => ({ case_id: c.case_id, case_name: c.case_name, batch_count: c.batch_count })));
        const next = caseList.items[0]?.case_id ?? null;
        if (next && next !== id) {
  ...
  }, [caseId, refreshPersons, refreshCandidates, setSearchParams]);
  ...
  const addManualLink = async () => {
    if (!caseId || !manualPersonId) return;
    ...
    () => [
      {
        ...
        width: 100,
        render: (v: string) => (
          <Tag color="volcano">{IDENTIFIER_TYPE_LABELS[v] || v}</Tag>
        ),
      },
      {
        title: "标识值",
  ...
  return (
    <div className="link-page">
      ...
      <Card className="app-card link-hero" bordered={false}>
        <div className="link-hero-inner">
          ...
          <Space wrap className="link-hero-actions">
            ...
            />
            <Button icon={<ScanOutlined />} onClick={() => void discover()} disabled={!caseId}>
              扫描候选
            </Button>
            <Button type="primary" icon={<ThunderboltOutlined />} onClick={() => void autoLink()} disabled={!caseId}>
              机器预关联
            </Button>
            <Button icon={<PlusOutlined />} onClick={() => setCreateOpen(true)}>
              新建人物
            </Button>
            <Button type="primary" ghost onClick={() => navigate(`/fusion-cockpit?case=${caseId || ""}`)} disabled={!caseId}>
              融合驾驶舱
            </Button>
          </Space>
        </div>
      ...
      >
        ...
        <Table
          ...
          scroll={{ x: 860 }}
          locale={{ emptyText: "暂无待处理候选，请先绑定批次并扫描" }}
        />
      </Card>
