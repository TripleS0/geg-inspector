...
export function useDataImportForm(options: UseDataImportFormOptions = {}) {
  ...
  const syncBatchNameFromFiles = (type: SourceType, fileList: UploadFile[]) => {
    const nextAutoName =
      type === "commercial"
        ? `商务网-${dayjs().format("YYYY-MM-DD-HHmmss")}`
        : fileList[0]?.name
          ? fileNameWithoutExt(fileList[0].name)
          : "";
  ...
  const getPayload = async (): Promise<DataImportPayload> => {
    ...
    return {
      values: { ...values, batch_name: batchName },
      files: realFiles,
      bankImportMode: useOcr ? "ocr" : "excel",
      sheetAssignments: values.source_type === "bank" ? bankAssignments : undefined,
    };
  };
  ...
  const formElement = useMemo(
    ...
    [compact, disabled, filesBySource, form, isCommercial, uploadSection]
  );
